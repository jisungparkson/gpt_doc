...
PROMPTS = {
    ...
    "plan": ChatPromptTemplate.from_template("""
        ...
        - 반드시 '1. 관련:' 항목부터 시작하세요
        - 순수한 공문서 형식으로만 작성하세요
        - 붙임 항목 정렬 (중요): "붙임 1."로 시작하고, 두 번째부터는 5칸 띄운 후 "2. 파일명 1부. 끝." 형식으로 세로줄을 정확히 맞춰 정렬
        ...
        - 붙임: 첨부 파일이 2개 이상이면 각 파일명에 번호를 붙여 나열하고, 각 파일명 끝에 수량을 표기합니다. 첫 번째는 "붙임 1.", 두 번째부터는 5칸 띄우고 "2." 형식으로 정렬합니다.
        - 붙임 정렬 규칙: 첫 번째 항목은 "붙임" 뒤에 공백 후 "1."로 시작하고, 두 번째 항목부터는 "붙임"의 "ㅁ" 아래에 정확히 맞춰 번호를 정렬합니다. 마지막 항목 끝에 "끝."을 붙입니다.
        ...
        5. 붙임: 붙임 문서 목록을 정확히 기재하고, 목록 끝에 '1부.'와 같이 수량을 표기한 후, 전체 문서의 마지막은 '끝.'으로 마무리합니다. 
           형식: "붙임 1. 파일명 1부." (첫 번째), "     2. 파일명 1부. 끝." (두 번째)
    ...
    with record_tab1:
        ...
        if 'generated_table_results' in st.session_state and st.session_state.generated_table_results:
            with st.container(border=True):
                                    ...
                                    <td class="record-cell">
                                        <div class="record-content">
                                            <div class="record-text" id="text_{i}">
                                                {escaped_text}
                                                <span class="char-info">{total_chars}자 / {nice_bytes}bytes</span>
                                            </div>
                ...
                # 편집할 학생 선택
                student_names = [data['name'] for data in st.session_state.generated_table_results]
                selected_student = st.selectbox("편집할 학생 선택", ["선택하세요"] + student_names)
                
                if selected_student and selected_student != "선택하세요":
                    selected_index = student_names.index(selected_student)
                    original_text = st.session_state.generated_table_results[selected_index]['result']
                    
                    edited_text = st.text_area(
                        f"{selected_student} 교과세특 편집",
                        value=original_text,
                        height=150,
                        key=f"edit_{selected_index}"
                    )
                    
                    if st.button("편집 내용 저장", key=f"save_{selected_index}"):
                        st.session_state.generated_table_results[selected_index]['result'] = edited_text
                        st.success(f"{selected_student}의 교과세특이 수정되었습니다!")
                        st.rerun()
                
                # 전체 결과 복사 버튼
                st.markdown("---")
                all_results = "\n\n".join([f"{i+1:02d}. {data['name']}\n{data['result']}" for i, data in enumerate(st.session_state.generated_table_results)])
                ...
                st.components.v1.html(all_copy_button_html, height=80)
        ...
        with st.container(border=True):
            ...
            if st.button("✨ 행발 예시 생성", use_container_width=True, key="behavior_record"):
                if all([behavior_trait, observation_behavior, growth_point]):
                    # 고유한 세션 키 생성
                    session_key = f"behavior_record_{hash(behavior_trait + observation_behavior)}"
                    run_chain_and_display(session_key, "behavior_record", {"behavior_trait": behavior_trait, "observation": observation_behavior, "growth_point": growth_point}, st)

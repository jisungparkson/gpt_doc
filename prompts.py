...
PROMPTS = {
    ...
    "student_record": ChatPromptTemplate.from_template("""
        ...
        [매우 중요한 작성 규칙]
        1. 명사형 종결: 모든 문장은 반드시 '~함.', '~음.', '~능력을 보임.' 과 같이 명사형 어미로 끝나야 한다. 절대로 '~습니다', '~합니다' 와 같은 서술형 어미를 사용해서는 안 된다.
        - (좋은 예시): 문제를 해결하는 능력이 뛰어남.
        - (나쁜 예시): 문제를 해결하는 능력이 뛰어납니다.
        2. 주어 생략: 문장에서 '학생은', '이 학생은' 과 같은 주어를 반드시 생략해야 한다. 주어 없이 서술하는 것이 원칙이다.
        - (좋은 예시): 다양한 의견을 조율하여 합의를 이끌어냄.
        - (나쁜 예시): 학생은 다양한 의견을 조율하여 합의를 이끌어냈습니다.
        3. 객관적 서술: 관찰된 사실을 바탕으로 객관적으로 서술해야 한다.

        [출력 예시]
        '모둠 활동에서 적극적으로 의견을 제시하고, 친구의 말을 경청하는 태도를 보임. 특히, 복잡한 문제 상황에서 창의적인 해결책을 제시하여 문제 해결에 크게 기여함. 이러한 과정을 통해 소통 역량과 문제해결역량이 크게 성장함.'
        ...
        - 성취기준: {achievement}
        ---
        위의 모든 규칙과 예시를 완벽하게 준수하여, 아래 관찰 내용을 바탕으로 학생의 특성을 서술하라. 성취기준이 제공된 경우 이를 적절히 반영하여 작성해 주세요."""),
    ...
    "behavior_record": ChatPromptTemplate.from_template("""
        ...
        4. 인성 역량 연결: 관찰된 행동을 '소통역량', '공동체역량', '자기관리역럅' 등과 연결하여 의미를 부여합니다.
        ...
        ---
        위의 모든 규칙과 예시를 완벽하게 준수하여, 아래 관찰 내용을 바탕으로 학생의 특성을 서술하라."""),
    ...
    # (신규) 스마트 기안문 프롬프트 - 모든 유형을 동적으로 처리
    "smart_giamun": ChatPromptTemplate.from_template("""
당신은 대한민국 공문서(기안문)의 구조를 완벽하게 이해하고, 그 내용을 구조화된 JSON 데이터로 생성하는 전문가입니다.
사용자가 선택한 기안문 유형에 맞춰 완벽한 공문서 형식의 JSON을 생성해야 합니다.

# PERFECT EXAMPLES (사용자의 '기안 종류'에 맞는 예시를 선택하여 따르세요)

## EXAMPLE 1: "내부결재: 각종 계획 수립"
- **최종 문서 예시:**
    제목: 2025학년도 5학년 현장체험학습 실시 계획(안)
    1. 관련: 2025학년도 학교교육계획
    2. 위 호와 관련하여 ... 현장체험학습을 다음과 같이 실시하고자 합니다.
       가. 일시: ...
       나. 장소: ...
    붙임  1. 세부 일정 1부. 끝.
- **JSON 구조:** {{ "related_basis": [...], "introduction": "...", "items": [{{...}}], "attachments": [...] }}

## EXAMPLE 2: "내부결재: 예산 집행(품의)"
- **최종 문서 예시:**
    제목: 교실용 전자칠판 구입 품의
    1. 관련: 2025학년도 학교회계 예산서
    2. 위 호와 관련하여 ... 구입하고자 다음과 같이 품의합니다.
       가. 품명: 85인치 전자칠판
       나. 수량: 2대
       다. 금액: 금2,500,000원
    붙임  1. 견적서 1부. 끝.
...
## EXAMPLE 3: "대외발송: 자료 제출" 
- **최종 문서 예시:**
    수신: 교육지원청 교육장
    제목: 2025학년도 학교폭력 실태조사 결과 제출
    1. 관련: 교육지원청 OOO과-1234호, "학교폭력 실태조사 실시"
    2. 위 호에 따라 ... 결과를 다음과 같이 제출합니다.
       가. 조사 기간: ...
       나. 조사 결과: ...
    붙임  1. 실태조사 결과 보고서 1부. 끝.
- **JSON 구조:** {{ "recipient": "...", "related_basis": [...], "introduction": "...", "items": [{{...}}], "attachments": [...] }}

## EXAMPLE 4: "결과 보고서"
- **최종 문서 예시:**
    제목: 2025학년도 현장체험학습 결과 보고
    1. 관련: OOO-1234, "2025학년도 현장체험학습 계획"
    2. 위 호에 따라 실시한 현장체험학습 결과를 다음과 같이 보고합니다.
       가. 활동 개요: ...
       나. 예산 정산 결과: ...
       다. 평가 및 제언: ...
    붙임  1. 활동 사진 1부. 끝.
...
- **첨부 파일:** {user_input.get('attachments', '')}

# JSON 출력 구조 (반드시 이 형식을 준수하라)
{{
    "recipient": "수신자 (대외발송일 때만)",
    "related_basis": ["관련 근거 1", "관련 근거 2", ...],
    "introduction": "본문 시작 문장 (예: '위 호와 관련하여 ... 다음과 같이 실시하고자 합니다.')",
    "items": [
        {{
            "content": "항목 내용",
            "sub_items": [
    ...
    "attachments": ["첨부파일1 1부", "첨부파일2 1부", ...]
}}

# 중요한 규칙
1. **반드시 유효한 JSON 형식으로만 응답하라. 다른 설명이나 텍스트는 일체 포함하지 마라.**
2. **기안 종류에 따라 적절한 구조와 내용을 생성하라:**
   - "내부결재: 각종 계획 수립" → 계획 세부사항, 일시/장소/대상/예산 등
   - "내부결재: 예산 집행(품의)" → 품명, 수량, 금액, 예산과목 등
   - "대외발송: 자료 제출" → recipient 포함, 제출 자료명과 관련 공문번호
   - "결과 보고서" → 활동 개요, 실행 결과, 평가 및 제언 등
3. **related_basis는 해당 기안의 법적 근거나 관련 규정을 포함하라.**
4. **introduction은 기안의 목적과 배경을 간단히 설명하는 문장이어야 한다.**
5. **items는 기안의 핵심 내용을 구조화된 형태로 작성하라.**
6. **하위 항목(sub_items)은 필요시에만 사용하고, 깊이는 최대 3단계까지만 허용한다.**
7. **attachments는 실제 첨부파일명을 포함하며, 각 항목 끝에 '1부' 등의 수량을 포함하라.**
